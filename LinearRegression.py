...
class LinearRegression:
    ...
    #here we are initialising the weights and bias ie 0
        n_samples, n_features = X.shape #to know how many features and samples we are having in our dataset
        ...
        self.weights = np.zeros((n_features,1)) #created weights for each feature having initial value as 0
        self.bias = 0
        ...
        for _ in range(self.n_iters): #looping through the number of iterations
            ...
            #gradient
            dw = (1/n_samples) * np.dot(X.t, (y_pred-y).reshape(-1,1)) #the dot thing also does the sum and X.T ensures correct matrix multiplication
